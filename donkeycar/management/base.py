...
class CreateCar(BaseCommand):
    def parse_args(self, args):
        ...
        parser.add_argument(
            "--path", default=None, help="path where to create car folder"
        )
        parser.add_argument(
            "--template", default=None, help="name of car template to use"
        )
        parser.add_argument(
            "--overwrite",
            action="store_true",
    ...
    def run(self, args):
        ...
        self.create_car(
            path=args.path, template=args.template, overwrite=args.overwrite
        )
...
class CalibrateCar(BaseCommand):
    def parse_args(self, args):
        ...
        parser.add_argument(
            "--channel", help="The channel you'd like to calibrate [0-15]"
        )
        parser.add_argument(
            "--address",
            default="0x40",
        ...
        parser.add_argument(
            "--bus",
            default=None,
            help="The i2c bus you'd like to calibrate [default autodetect]",
        )
        parser.add_argument(
            "--pwmFreq", default=60, help="The frequency to use for the PWM"
        )
        parsed_args = parser.parse_args(args)
        return parsed_args
    ...
    def run(self, args):
        ...
        print(
            "init PCA9685 on channel %d address %s bus %s"
...
class MakeMovieShell(BaseCommand):
    ...
    def parse_args(self, args):
        ...
        parser.add_argument(
            "--type", default=None, help="the model type to load"
        )
        parser.add_argument(
            "--salient",
            action="store_true",
        ...
        parser.add_argument(
            "--start", type=int, default=0, help="first frame to process"
        )
        parser.add_argument(
            "--end", type=int, default=-1, help="last frame to process"
        )
        parser.add_argument(
            "--scale",
            type=int,
...
class TubCheck(BaseCommand):
    def parse_args(self, args):
        ...
        parser.add_argument(
            "--fix", action="store_true", help="remove problem records"
        )
        parser.add_argument(
            "--delete_empty",
            action="store_true",
...
class ShowHistogram(BaseCommand):
    def parse_args(self, args):
        ...
        parser.add_argument(
            "--record", default=None, help="name of record to create histogram"
        )
        parsed_args = parser.parse_args(args)
        return parsed_args
...
class ConSync(BaseCommand):
    ...
    def parse_args(self, args):
        parser = argparse.ArgumentParser(
            prog="consync", usage="%(prog)s [options]"
        )
        parser.add_argument(
            "--dir", default="./cont_data/", help="paths to tubs"
        )
        parser.add_argument(
            "--delete",
            default="y",
            help="remove files locally that were deleted remotely y=yes n=no",
        )
        parsed_args = parser.parse_args(args)
        return parsed_args
    ...
    def run(self, args):
        ...
        if args.delete == "y":
            reply = input(
                "WARNING:this rsync operation will delete data in the target dir: %s. ok to proceeed? [y/N]: "
                % dest_dir
            )
...
class ConTrain(BaseCommand):
    ...
    def parse_args(self, args):
        parser = argparse.ArgumentParser(
            prog="contrain", usage="%(prog)s [options]"
        )
        parser.add_argument(
            "--tub", default="./cont_data/*", help="paths to tubs"
        )
        parser.add_argument(
            "--model", default="./models/drive.h5", help="path to model"
        )
        parser.add_argument(
            "--transfer", default=None, help="path to transfer model"
        )
        parser.add_argument(
            "--type",
            default="categorical",
            help="type of model (linear|categorical|rnn|imu|behavior|3d)",
        )
        parser.add_argument(
            "--aug", action="store_true", help="perform image augmentation"
        )
        parsed_args = parser.parse_args(args)
        return parsed_args
...
class ShowPredictionPlots(BaseCommand):
    ...
    def parse_args(self, args):
        ...
        parser.add_argument("--tub", nargs="+", help="paths to tubs")
        parser.add_argument(
            "--model", default=None, help="name of record to create histogram"
        )
        parser.add_argument(
            "--limit", default=1000, help="how many records to process"
        )
