...
class CreateJoystick(object):
        ...
        while time.time() - start < duration:
            if self.last_axis:
                if self.last_axis in axis_samples:
                    try:
                        axis_samples[self.last_axis] = axis_samples[
                            self.last_axis
                        ] + math.fabs(self.axis_val)
                    except:
                        try:
                            axis_samples[self.last_axis] = math.fabs(
                                self.axis_val
                            )
                        except:
                            pass
    ...
    def find_gyro(self):
        ...
        while time.time() - start < 5.0:
            if (
                self.last_axis is not None
                and not self.last_axis in self.gyro_axis
            ):
        ...
        if len(self.gyro_axis) > 0:
            print(
                "Ok, we found %d axes that stream gyroscope data. We will ignore those during labelling and mapping."
                % len(self.gyro_axis)
            )
        else:
            print(
            ...
            if self.last_button is None:
                ...
                if code is not None:
                    if code in self.js.button_names:
                        ret = input(
                            "This button has a name: %s. Are you done naming? (y/N) "
                            % self.js.button_names[code]
                        )
                        if ret.upper() == "Y":
                            done = True
                            break
    ...
    def name_axes(self):
        ...
        while not done:
            ...
            if "unknown" in most_movement:
                ...
                if code is not None:
                    label = input(
                        "What name to give to this axis: (D when done) "
                    )
                    if len(label) == 0:
                        print("No name given. skipping.")
                    elif label.upper() == "D":
    ...
    def write_python_class_file(self):
        ...
        while pyth_filename is None:
            ...
            pyth_filename = input(
                "What is the name of python file to create joystick code? [default: my_joystick.py]"
            )
            ...
            try:
                ...
                ret = input(
                    "failed to open filename. Enter another filename? [Y,n]"
                )
                if ret == "n":
                    break
                pyth_filename = None
        ...
        if outfile is not None:
            classname = input(
                "What is the name of joystick class? [default: MyJoystick] "
            )
            if len(classname) == 0:
...
class %sController(JoystickController):
    ...
    def map_control_axis(self, control_name, control_fn):
        while True:
            axis = self.get_axis_action(
                "Move the controller axis you wish to use for %s. Continue moving for 2 seconds."
                % control_name
            )
    ...
    def map_button_controls(self):
        unmapped_controls = [
            (
                "toggle_mode",
                "changes the drive mode between user, local, and local_angle",
            ),
            (
                "erase_last_N_records",
                "erases the last 100 records while driving",
            ),
            (
                "emergency_stop",
                "executes a full back throttle to bring car to a quick stop",
            ),
            (
                "increase_max_throttle",
                "increases the max throttle, also used for constant throttle val",
            ),
            (
                "decrease_max_throttle",
                "decreases the max throttle, also used for constant throttle val",
            ),
            (
                "toggle_constant_throttle",
                "toggle the mode of supplying constant throttle",
            ),
        ...
        while len(unmapped_controls) > 0:
            ...
            try:
                ret = " "
                while (not ret.isdigit() and ret.upper() != "D") or (
                    ret.isdigit()
                    and (int(ret) < 1 or int(ret) > len(unmapped_controls))
                ):
                    ret = input(
                        "Press the number of control to map (1-%d). D when done. "
                        % len(unmapped_controls)
                    )
            ...
            print(
                "Press the button to map to control:",
                unmapped_controls[iControl][0],
            )
            self.get_button_press()
            ...
            if self.last_button is None:
                ...
                self.mapped_controls.append(
                    (button_name, unmapped_controls[iControl][0])
                )
    ...
    def parse_args(self, args):
        parser = argparse.ArgumentParser(
            prog="createjs", usage="%(prog)s [options]"
        )
