...
class SalientVis:
    ...
    def init_salient(self, model):
        ...
        x = img_in
        x = Convolution2D(
            24, (5, 5), strides=(2, 2), activation="relu", name="conv1"
        )(x)
        x = Convolution2D(
            32, (5, 5), strides=(2, 2), activation="relu", name="conv2"
        )(x)
        x = Convolution2D(
            64, (5, 5), strides=(2, 2), activation="relu", name="conv3"
        )(x)
        x = Convolution2D(
            64, (3, 3), strides=(2, 2), activation="relu", name="conv4"
        )(x)
        conv_5 = Convolution2D(
            64, (3, 3), strides=(1, 1), activation="relu", name="conv5"
        )(x)
    ...
    def compute_visualisation_mask(self, img):
        ...
        for layer in [5, 4, 3, 2, 1]:
            averaged_activation = (
                np.mean(activations[layer], axis=3).squeeze(axis=0)
                * upscaled_activation
            )
            output_shape = (
                activations[layer - 1].shape[1],
            ...
            x = tf.constant(
                np.reshape(
                    ...
                    (
                        1,
                        averaged_activation.shape[0],
                        averaged_activation.shape[1],
                        1,
                    ),
                ),
                tf.float32,
