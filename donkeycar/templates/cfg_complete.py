...
# (0 => none , 4 => Flip horizontally, 6 => Flip vertically)
CSIC_CAM_GSTREAMER_FLIP_PARM = 0

# 9865, over rides only if needed, ie. TX2..
PCA9685_I2C_ADDR = 0x40  # I2C address, use i2cdetect to validate this number

# None will auto detect, which is fine on the pi. But other platforms should specify the bus num.
...
# valid values: SERVO_ESC|DC_STEER_THROTTLE|DC_TWO_WHEEL|SERVO_HBRIDGE_PWM
...
# valid values: (linear|categorical|rnn|imu|behavior|3d|localizer|latent)
...
# password is optional. Only used from Windows machine. Ubuntu and mac users should copy their public keys to the pi. `ssh-copy-id username@hostname`
PI_PASSWD = "raspberry"

PI_HOSTNAME = "raspberrypi.local"  # the network hostname or ip address

# the location of the mycar dir on the pi. this will be used to help locate the final model destination.
...
# this scalar is multiplied with the -1 to 1 throttle value to limit the maximum throttle. This can help if you drop the controller or just don't need the full speed available.
JOYSTICK_MAX_THROTTLE = 0.5

# some people want a steering that is less sensitve. This scalar is multiplied with the steering -1 to 1. It can be negative to reverse dir.
JOYSTICK_STEERING_SCALE = 1.0

# if true, we will record whenever throttle is not zero. if false, you must manually toggle recording with some other trigger. Usually circle button on joystick.
...
# SOMBRERO
# set to true when using the sombrero hat from the Donkeycar store. This will enable pwm on the hat.
HAVE_SOMBRERO = False
...
HAVE_RGB_LED = (
    False
)  # do you have an RGB LED like https://www.amazon.com/dp/B07BNRZWNF

# COMMON ANODE? Some RGB LED use common anode. like https://www.amazon.com/Xia-Fly-Tri-Color-Emitting-Diffused/dp/B07MYJQP8B
LED_INVERT = False
...
# how many cycles of 1/20 of a second to blink per REC_COUNT_ALERT records
