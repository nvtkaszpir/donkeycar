...
class KerasPilot(object):
    ...
    def set_optimizer(self, optimizer_type, rate, decay):
        if optimizer_type == "adam":
            ...
            self.model.optimizer = keras.optimizers.RMSprop(
                lr=rate, decay=decay
            )
        else:
...
class KerasCategorical(KerasPilot):
    ...
    def run(self, img_arr):
        ...
        N = len(throttle[0])
        throttle = dk.utils.linear_unbin(
            throttle, N=N, offset=0.0, R=self.throttle_range
        )
...
class KerasLocalizer(KerasPilot):
    ...
    def compile(self):
        self.model.compile(
            optimizer=self.optimizer, metrics=["acc"], loss="mse"
        )
...
class KerasRNN_LSTM(KerasPilot):
    ...
    def run(self, img_arr):
        ...
        img_arr = np.array(self.img_seq).reshape(
            1, self.seq_length, self.image_h, self.image_w, self.image_d
        )
        outputs = self.model.predict([img_arr])
...
def default_latent(num_outputs, input_shape):
    ...
    x = Dropout(drop)(x)
    x = Convolution2D(
        32, (5, 5), strides=(2, 2), activation="relu", name="conv2d_3"
    )(x)
    x = Dropout(drop)(x)
    x = Convolution2D(
        32, (3, 3), strides=(1, 1), activation="relu", name="conv2d_4"
    )(x)
    x = Dropout(drop)(x)
    x = Convolution2D(
        32, (3, 3), strides=(1, 1), activation="relu", name="conv2d_5"
    )(x)
    x = Dropout(drop)(x)
    x = Convolution2D(
        64, (3, 3), strides=(2, 2), activation="relu", name="conv2d_6"
    )(x)
    x = Dropout(drop)(x)
    x = Convolution2D(
        64, (3, 3), strides=(2, 2), activation="relu", name="conv2d_7"
    )(x)
    x = Dropout(drop)(x)
    x = Convolution2D(
        10, (1, 1), strides=(2, 2), activation="relu", name="latent"
    )(x)

    y = Conv2DTranspose(
        filters=64, kernel_size=(3, 3), strides=2, name="deconv2d_1"
    )(x)
    y = Conv2DTranspose(
        filters=64, kernel_size=(3, 3), strides=2, name="deconv2d_2"
    )(y)
    y = Conv2DTranspose(
        filters=32, kernel_size=(3, 3), strides=2, name="deconv2d_3"
    )(y)
    y = Conv2DTranspose(
        filters=32, kernel_size=(3, 3), strides=2, name="deconv2d_4"
    )(y)
    y = Conv2DTranspose(
        filters=32, kernel_size=(3, 3), strides=2, name="deconv2d_5"
    )(y)
    y = Conv2DTranspose(
        filters=1, kernel_size=(3, 3), strides=2, name="img_out"
    )(y)
