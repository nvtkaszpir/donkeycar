...
class Tub(object):
    ...
    def update_df(self):
        df = pd.DataFrame(
            [self.get_json_record(i) for i in self.get_index(shuffled=False)]
        )
        self.df = df
    ...
    def put_record(self, data):
        ...
        for key, val in data.items():
            ...
            if (val is not None) and (typ == "float"):
                ...
                msg = "Tub does not know what to do with this type {}".format(
                    typ
                )
                raise TypeError(msg)
    ...
    def get_json_record(self, ix):
        ...
        try:
            ...
            raise Exception(
                "bad record: %d. You may want to run `python manage.py check --fix`"
                % ix
            )
        except FileNotFoundError:
            raise
    ...
    def gather_records(self):
        ...
        if len(self.exclude) > 0:
            record_paths = [
                f for f in record_paths if ri(f) not in self.exclude
            ]
    ...
    def get_batch_gen(
        self, keys, record_transform=None, batch_size=128, shuffle=True, df=None
    ):

        record_gen = self.get_record_gen(
            record_transform, shuffle=shuffle, df=df
        )
    ...
    def get_train_gen(
        self, X_keys, Y_keys, batch_size=128, record_transform=None, df=None
    ):
...
class TubHandler:
    ...
    def new_tub_writer(self, inputs, types, user_meta=[]):
        ...
        tw = TubWriter(
            path=tub_path, inputs=inputs, types=types, user_meta=user_meta
        )
        return tw
    ...
    def get_record(self, ix):
        ...
        if ix > 1:
            ...
            for key, val in json_data.items():
                ...
                # load objects that were saved as separate files
                if typ == "image":
                    val = self.stack3Images(
                        data_ch0[key], data_ch1[key], data[key]
                    )
                    data[key] = val
                elif typ == "image_array":
                    img = self.stack3Images(
                        data_ch0[key], data_ch1[key], data[key]
                    )
                    val = np.array(img)
...
class TubGroup(Tub):
    def __init__(self, tub_paths):
        ...
        print(
            "joining the tubs {} records together. This could take {} minutes.".format(
                record_count, int(record_count / 300000)
            )
        )

        self.meta = {
            "inputs": list(self.input_types.keys()),
