...
def drive(
    ...
    if camera_type == "stereo":
        ...
        if cfg.DONKEY_GYM:
            ...
            cam = DonkeyGymEnv(
                cfg.DONKEY_SIM_PATH, env_name=cfg.DONKEY_GYM_ENV_NAME
            )
            threaded = True
        ...
        V.add(
            cam, inputs=inputs, outputs=["cam/image_array"], threaded=threaded
        )
    ...
    if cfg.HAVE_RGB_LED and not cfg.DONKEY_GYM:
        ...
        led = RGB_LED(
            cfg.LED_PIN_R, cfg.LED_PIN_G, cfg.LED_PIN_B, cfg.LED_INVERT
        )
    ...
    V.add(
        rec_tracker_part, inputs=["tub/num_records"], outputs=["records/alert"]
    )
    ...
    def load_model_json(kl, json_fnm):
        ...
        try:
            ...
            print(
                "finished loading json in %s sec." % (str(time.time() - start))
            )
        except Exception as e:
            print(e)
    ...
    if model_path:
        ...
        V.add(
            FileWatcher(model_path, verbose=True),
    ...
    class DriveMode:
        def run(
            self, mode, user_angle, user_throttle, pilot_angle, pilot_throttle
        ):
            if mode == "user":
        ...
        if cfg.BUTTON_PRESS_NEW_TUB:
            ...
            def new_tub_dir():
                ...
                tub = th.new_tub_writer(
                    inputs=inputs, types=types, user_meta=meta
                )
                V.add(
                    tub,
                    inputs=inputs,
