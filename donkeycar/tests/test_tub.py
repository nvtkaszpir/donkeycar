...
class TestTubWriter(unittest.TestCase):
    ...
    def test_tub_meta(self):
        ...
        tub = Tub(
            self.path, inputs=["file_path"], types=["image"], user_meta=meta
        )
        t2 = Tub(self.path)
    ...
    def test_tub_like_driver(self):
        ...
        tub = th.new_tub_writer(
            inputs=self.inputs, types=self.types, user_meta=meta
        )
        t2 = Tub(tub.path)
