...
from donkeycar.parts.cv import (
    CvImageView,
    ImgBGR2RGB,
    ImgRGB2BGR,
    ImageScale,
    ImgWriter,
    ArrowKeyboardControls,
)
...
V.add(
    MQTTValueSub(
        name="donkey/%s/camera" % args["--name"], broker=args["--broker"]
    ),
    outputs=["jpg"],
)
...
V.add(
    MQTTValuePub(name="donkey/%s/controls" % args["--name"]), inputs=["control"]
)
