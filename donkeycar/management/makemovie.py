...
class MakeMovie(object):
    ...
    def draw_steering_distribution(self, record, img):
        ...
        if (
            self.keras_part is None
            or type(self.keras_part) is not KerasCategorical
        ):
            return
    ...
    def make_frame(self, t):
        ...
        if self.cfg.ROI_CROP_TOP != 0 or self.cfg.ROI_CROP_BOTTOM != 0:
            image = img_crop(
                image, self.cfg.ROI_CROP_TOP, self.cfg.ROI_CROP_BOTTOM
            )
        ...
        if self.scale != 1:
            ...
            image = cv2.resize(
                image, dsize=dsize, interpolation=cv2.INTER_CUBIC
            )
