...
def train(
    ...
    if cfg.OPTIMIZER:
        kl.set_optimizer(
            cfg.OPTIMIZER, cfg.LEARNING_RATE, cfg.LEARNING_RATE_DECAY
        )
                ...
                if len(batch_data) == batch_size:
                    ...
                    for record in batch_data:
                        ...
                        if img_out:
                            ...
                            out_img.append(
                                rz_img_arr[:, :, 0].reshape((127, 127, 1))
                            )
...
def go_train(
    ...
    if cfg.SHOW_PLOT:
        try:
            if do_plot:
                ...
                plt.savefig(
                    model_path
                    + "_loss_acc_%f.%s" % (save_best.best, figure_format)
                )
                plt.show()
            else:
    ...
    if "tflite" in cfg.model_type:
        ...
        keras_model_to_tflite(
            model_path, tflite_fnm, representative_dataset_gen
        )
    ...
    if cfg.PRUNE_CNN:
        ...
        target_channels = int(
            cnn_channels * (1 - (float(cfg.PRUNE_PERCENT_TARGET) / 100.0))
        )

        print(
            "Target channels of {0} remaining with {1:.00%} percent removal per iteration".format(
                target_channels, cfg.PRUNE_PERCENT_PER_ITERATION / 100
            )
        )
        ...
        print(
            "pruning stopped at {} with a target of {}".format(
...
def prune(model, validation_generator, val_steps, cfg):
    ...
    name = "{}/model_pruned_{}_percent.h5".format(
        cfg.MODELS_PATH, percent_pruning
    )
...
def extract_data_from_pickles(cfg, tubs):
    ...
    for tub_path in t_paths:
        ...
        for file_path in file_paths:
            ...
            with open(
                join(base_path, "record_{}.json".format(filename)), "w"
            ) as f:
...
def prune_model(model, apoz_df, n_channels_delete):
    ...
    for name in high_apoz_index.index.unique().values:
        ...
        surgeon.add_job(
            "delete_channels", model.get_layer(name), channels=channels
        )
    # Delete channels
...
def get_model_apoz(model, generator):
            ...
            apoz.extend(
                [
                    ...
                    for (i, value) in enumerate(
                        get_apoz(model, layer, generator)
                    )
                ]
            )
    ...
    apoz_df = pd.DataFrame(
        {"layer": layer_name, "index": index, "apoz": apoz_value}
    )
