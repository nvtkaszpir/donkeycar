...
class PWMThrottle:
    ...
    def __init__(
        self, controller=None, max_pulse=300, min_pulse=490, zero_pulse=350
    ):
...
class Maestro:
    ...
    def __init__(self, channel, frequency=60):
        ...
        if Maestro.astar_device == None:
            Maestro.astar_device = serial.Serial(
                "/dev/ttyACM2", 115200, timeout=0.01
            )
    ...
    def set_pulse(self, pulse):
        ...
        with Maestro.maestro_lock:
            Maestro.maestro_device.write(
                bytearray([0x84, self.channel, (w & 0x7F), ((w >> 7) & 0x7F)])
            )
...
class Teensy:
    ...
    def __init__(self, channel, frequency=60):
        ...
        if Teensy.teensy_device == None:
            Teensy.teensy_device = serial.Serial(
                "/dev/teensy", 115200, timeout=0.01
            )
        ...
        if Teensy.astar_device == None:
            Teensy.astar_device = serial.Serial(
                "/dev/astar", 115200, timeout=0.01
            )
...
class L298N_HBridge_DC_Motor(object):
    ...
    def run(self, speed):
        ...
        self.throttle = int(
            dk.utils.map_range(speed, -1, 1, -max_duty, max_duty)
        )
...
class TwoWheelSteeringThrottle(object):
    def run(self, throttle, steering):
        if throttle > 1 or throttle < -1:
            raise ValueError(
                "throttle must be between 1(forward) and -1(reverse)"
            )
...
class Mini_HBridge_DC_Motor_PWM(object):
    ...
    def run(self, speed):
        ...
        self.speed = speed
        self.throttle = int(
            dk.utils.map_range(speed, -1, 1, -self.max_duty, self.max_duty)
        )
